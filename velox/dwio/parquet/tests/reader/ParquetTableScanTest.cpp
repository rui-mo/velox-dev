...
// The filter references the whole struct, but no child of the struct is
// projected or filtered. The reader still needs one hidden leaf reader to
// source the struct's repetition/definition levels.
TEST_F(ParquetTableScanTest, structMatchByIndexStructOnlyFilter) {
  ...
  const auto dataColumns =
      ROW({"id", "name", "address"},
          {BIGINT(), ROW({"first", "last"}, VARCHAR()), VARCHAR()});
  const auto outputType = ROW({"id"}, {BIGINT()});
  const auto plan =
      PlanBuilder()
          .tableScan(outputType, {}, "not(is_null(name))", dataColumns)
          .planNode();
  const auto expected = makeRowVector({"id"}, {id});
