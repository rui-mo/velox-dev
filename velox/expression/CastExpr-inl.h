...
void CastExpr::applyNumericUpcast(
  ...
  if constexpr (isNumericType(ToKind) && isNumericType(FromKind)) {
    ...
    if (input.isFlatEncoding()) {
      ...
      //   if (rows.isValid(i)) {
      //     // Converting large bigint/hugeint values to float/double directly
      //     may
      //     // lose precision, but it's consistent with the implementation in
      //     // velox/type/Conversions.h.
